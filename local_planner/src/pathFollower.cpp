...
#include <geometry_msgs/Twist.h>  // 修改: TwistStamped -> Twist
...
double minRotateSpeed = 0.0;  // 旋转时最小前进速度，>0 可激活 trot 步态防止腿冻住
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("minRotateSpeed", minRotateSpeed);
  ...
  // 修改: TwistStamped -> Twist
  ros::Publisher pubSpeed = nh.advertise<geometry_msgs::Twist> ("/cmd_vel", 5);
  geometry_msgs::Twist cmd_vel;
  // 移除: cmd_vel.header.frame_id = "vehicle"; (Twist 类型没有 header)
  ...
  while (status) {
    ...
    if (pathInit) {
      ...
      pubSkipCount--;
      if (pubSkipCount < 0) {
        // 旋转时若 vehicleSpeed ≈ 0 但仍有 yawRate，发送 minRotateSpeed 激活 trot 步态
        if (fabs(vehicleSpeed) <= maxAccel / 100.0) {
          if (minRotateSpeed > 0 && fabs(vehicleYawRate) > 0.01)
            cmd_vel.linear.x = minRotateSpeed;
          else
            cmd_vel.linear.x = 0;
